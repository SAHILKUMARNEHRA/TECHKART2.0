    ...
    <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
      <div className="lg:hidden">
        <div className="flex items-center justify-between gap-3">
          <div className="font-display text-2xl font-bold tracking-tight text-slate-900">Products</div>
      ...
      <aside className="hidden lg:block">
        <div className="sticky top-20 rounded-3xl border border-tk-border bg-white/70 p-6 shadow-soft backdrop-blur-sm">
          <div className="flex items-center justify-between">
            <div className="font-display text-lg font-bold text-slate-900">Filters</div>
      ...
      <main className="space-y-6">
        <div className="hidden items-end justify-between gap-4 lg:flex">
          <div>
            <div className="font-display text-3xl font-bold tracking-tight text-slate-900">Products</div>
            <div className="mt-1 text-sm font-medium text-slate-600">
              {status === 'success' ? `${filtered.length} results` : 'Loading catalog...'}
            </div>
          </div>
          <label className="flex items-center gap-3 rounded-2xl border border-tk-border bg-white/70 px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm backdrop-blur-sm transition hover:bg-white hover:shadow-md">
            Sort by:
            <select
              value={sort}
        ...
        <div className="lg:hidden">
          <label className="flex items-center justify-between gap-2 rounded-2xl border border-tk-border bg-white/70 px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm backdrop-blur-sm">
            Sort by
            <select
              value={sort}
              onChange={(e) => setSort(e.target.value as SortKey)}
      ...
      {isMobileFiltersOpen ? (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <div className="absolute inset-x-0 bottom-0 max-h-[86vh] overflow-auto rounded-t-3xl border border-tk-border bg-white/95 p-6 shadow-lift backdrop-blur-xl">
            <div className="flex items-center justify-between gap-3">
              <div className="font-display text-xl font-bold text-slate-900">Filters</div>
              <Button variant="secondary" onClick={() => setIsMobileFiltersOpen(false)} className="h-10">Done</Button>
            </div>
            <div className="mt-6">
              <ListingFilters
                priceBounds={priceBounds}
