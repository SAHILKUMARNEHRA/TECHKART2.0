    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-start">
      <div className="rounded-3xl border border-tk-border bg-white/70 p-6 shadow-soft backdrop-blur-sm">
        <div className="flex items-end justify-between gap-4">
          <div>
            <div className="font-display text-3xl font-bold tracking-tight text-slate-900">Cart</div>
            <div className="mt-1 text-sm font-medium text-slate-600">{items.length} item(s)</div>
          </div>
          <button
            className="inline-flex h-10 items-center justify-center rounded-xl border border-tk-border bg-white/80 px-5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-white hover:shadow-md"
            onClick={clear}
          >
            Clear
          </button>
        ...
        <div className="mt-6 grid gap-4">
          {lines.map((l) => (
            <div key={l!.product.id} className="rounded-2xl border border-tk-border bg-white/80 p-5 shadow-sm transition hover:bg-white">
              <div className="flex gap-5">
                ...
                <div className="min-w-0 flex-1">
                  <Link to={`/products/${l!.product.id}`} className="tk-clamp-2 text-base font-bold text-slate-900 hover:text-tk-primary transition-colors">
                    {l!.product.title}
                  </Link>
                  <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                    <div className="text-lg font-bold text-slate-900">{formatInr(l!.unit)}</div>
                    <button
                      className="inline-flex items-center gap-2 rounded-xl border border-tk-border bg-white/80 px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-white hover:shadow-md"
      ...
      <div className="sticky top-24 space-y-6">
        <div className="rounded-3xl border border-tk-border bg-white/70 p-6 shadow-soft backdrop-blur-sm">
          <div className="font-display text-xl font-bold text-slate-900">Summary</div>
          ...
          <div className="mt-6">
            <Link to="/checkout">
              <Button className="h-12 w-full">Checkout</Button>
            </Link>
