    ...
    <div className="grid gap-3 lg:grid-cols-[84px_1fr]">
      <div className="order-2 flex gap-2 overflow-auto lg:order-1 lg:flex-col lg:pr-1">
        {list.map((src) => (
          ...
          >
            <img src={src} alt={title} className="h-full w-full object-contain p-3" loading="lazy" />
          </button>
        ))}
      </div>
      ...
      >
        <img src={active} alt={title} className={cn('h-full w-full object-contain p-8', isHover ? 'opacity-0' : 'opacity-100')} />
        <div
